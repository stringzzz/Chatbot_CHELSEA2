...
Known issue: (STILL) Need to clean up the regexes using VERBOSE. Attempted to change all of them in an automated way and it just created many silent errors,
too much of a mess to clean up. It likely would have required testing every single part of the code again to find these hidden problems, and that's something
I wasn't willing to do. I will likely revisit this is a future version, but for now the regexes will remain the unorganized mess they've been in for a long time.
