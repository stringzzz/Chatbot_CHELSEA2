...
class date_difference:
	...
	def get_years_past(self, date):

		start = datetime.strptime(date, "%m/%d/%Y, %H:%M:%S")
		end = datetime.now()

		# Initial year subtraction
		years = end.year - start.year

		# Adjust down by 1 if the current date is before the anniversary
		has_anniversary_passed = (end.month, end.day, end.hour, end.minute) >= (start.month, start.day, start.hour, start.minute)
	...
	def get_time_greeting(self):
		...
		#Add more variety of greetings later

		time_match = re.search(r'(?P<hours>\d\d):\d\d:\d\d', datetime.now().strftime("%m/%d/%Y, %H:%M:%S"))
		if time_match:

			time = int(time_match.group('hours'))

			if time >= 3 and time < 12:
				
				#Morning
				return random.choice(['Good morning', 'Good morning to you', 'Top of the morning to you'])
			
			elif time >= 12 and time < 17:
				
				#Afternoon
				return random.choice(['Good afternoon', 'Good afternoon to you'])
			
			elif time >= 17 and time < 21:
				
				#Evening
				return random.choice(['Good evening', 'Good evening to you'])
			
			elif time >= 21 or time < 3:
				
				#Night
				return random.choice(['I see you\'re up late', 'Night time is the right time, I suppose'])

		else:

			return ''
		
	def get_holiday_greeting(self):

		if self.match_date('01/01'):

			#New Year's
			return ' Happy New Year\'s!'
		
		elif self.match_date('12/25'):

			#Christmas
			return ' Merry Christmas!'
		
		elif self.match_date('02/02'):

			#Groundhog Day
			return ' I feel like I\'ve done this before... Happy Groundhog Day!'
		
		elif self.match_date('03/17'):

			#St. Patrick's Day
			return ' Happy St. Patrick\'s Day!'
		
		elif self.match_date('04/01'):

			#April Fool's Day
			return random.choice([
				' I set your hard drive to format itself... April Fool\'s!', 
				' I donated the contents of your bank account to fund exploration of Uranus... April Fool\'s!', 
				' I changed your cryptocurrency wallet password to something random... April Fool\'s!'
			])
		
		elif self.match_date('05/05'):
