...
Now has other ways to try and find matching questions or messages given a specified similarity score (fuzzy matching).
Was able to actually reduce her code while making her a bit smarter, a rare win-win situation.
...
'filter': Prevents CHELSEA from saying certain 'inappropriate' words by replacing them with 'derpity'.
The only purpose for this is to do desktop recordings of the chats for uploading to somehwere like YouTube,
as there may be issues with her saying certain things. Note that it barely filters anything right now,
will add more to it later when needed.

############################################
####### Completely Optional Features #######
############################################

'netdancer': Has CHELSEA start an nmap network scan from the start of the chat.
Can continue talking to her like normal until the scan is complete, then can also ask questions about the network
or ask to start listening in on a specified device.